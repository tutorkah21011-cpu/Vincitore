...
-- ══════════════════════════════════════════════════
-- VINCITORE DUELS — MAIN VALIDATION (RE-CHECK)
-- ══════════════════════════════════════════════════
...
local userId   = _tostring(lp.UserId)
local username = lp.Name

-- ── HWID ───────────────────────────────────────────
local hwid = "UNKNOWN"
_pcall(function() hwid = _tostring(gethwid()) end)

if hwid == "UNKNOWN" or hwid == "" or #hwid <= 4 then
    lp:Kick("[Vincitore] Access Denied: HWID could not be verified.")
    return
end

-- ── Ambil token dari genv ──────────────────────────
local token = getgenv()._vincitore_token

-- Token wajib ada dan tepat 21 karakter
if _type(token) ~= "string" or #token ~= 21 then
    lp:Kick("[Vincitore] Access Denied: Invalid or missing token. Do not skip the loader.")
    return
end
...
if _type(script_key) ~= "string" or script_key == "" then
    lp:Kick("[Vincitore] Access Denied: Key missing on main script check.")
    return
end
...
local reqOk, response = _pcall(httpPost,
    ...
    .. "&hwid="       .. httpUrlEncode(hwid)
    .. "&userId="     .. userId
    .. "&username="   .. httpUrlEncode(username)
)

if not reqOk then
    lp:Kick("[Vincitore] Access Denied: Main script could not reach validation server.")
    return
end

if _type(response) ~= "string" or #response == 0 then
    lp:Kick("[Vincitore] Access Denied: Empty response on main validation.")
    return
end
...
_pcall(function()
    local data = httpDecode(response)
    valid = data.valid == true or data.success == true or false
    msg   = data.message or data.msg or msg
    if data.hwid and data.hwid ~= hwid then
        valid = false
        msg   = "HWID mismatch on main check."
    end
end)

if not valid then
    lp:Kick("[Vincitore] Access Denied: " .. msg)
    return
end

-- ── Bersihkan genv setelah validasi ───────────────
getgenv()._vincitore_token = nil
getgenv().script_key       = nil

_print("[Vincitore] ✔ Main validation passed — starting script...")
